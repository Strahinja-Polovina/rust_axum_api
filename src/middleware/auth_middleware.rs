use crate::messages::middleware_constants::{BEARER_WITH_SPACE, INVALID_HEADER_VALUE_ERROR, INVALID_TOKEN_ERROR, INVALID_TOKEN_FORMAT_ERROR, MISSING_AUTHORIZATION_HEADER_ERROR};
...
pub async fn middleware<B>(req: Request<B>, next: Next) -> Response {
        ...
        if let Ok(header_str) = header_value.to_str() {
            if header_str.starts_with(BEARER_WITH_SPACE) {
                ...
                match is_valid_token {
                    Ok(_) => next.run(Default::default()).await,
